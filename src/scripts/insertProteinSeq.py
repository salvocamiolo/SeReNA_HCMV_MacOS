...
# Form implementation generated from reading ui file 'insertProteinSeq.ui'
#
...
class Ui_Form(object):
	...
	def addSequence(self):
		...
		if os.path.isdir(installationDirectory+"/DB/"+self.sampleID_lineEdit.text()) == False:
			...
			msg.exec() 
			return
		if os.path.isfile(installationDirectory+"/DB/"+self.sampleID_lineEdit.text()+"/"+prefix+"_proteins.fasta") == False:
			msg = QtWidgets.QMessageBox()
			msg.setText("Corrupted entry! The protein file is not present in the specified sample")
			msg.exec() 
			return
		...
		admittedaminoAcids =["A","R","N","D","C","E","Q","G","H","I","L","K","M","F","P","S","T","W","Y","V"]
		position = 0
		for a in sequence.upper():
			position+=1
			if not a in admittedaminoAcids:
				msg = QtWidgets.QMessageBox()
		...
		foundSequence = ""
		sequences = {}
		for seq_record in SeqIO.parse(installationDirectory+"/DB/"+self.sampleID_lineEdit.text()+"/"+prefix+"_proteins.fasta","fasta"):
		...
		if geneExists == True:
			...
			if ret == QtWidgets.QMessageBox.No:
				...
				outfile = open(installationDirectory+"/DB/"+self.sampleID_lineEdit.text()+"/"+prefix+"_proteins.fasta","w")
	...
	def retranslateUi(self, Form):
		...
		self.label_6.setText(_translate("Form", "Protein sequence"))
